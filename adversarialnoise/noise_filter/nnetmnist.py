...
class NNetMnist:
	"""
	"""
	def __init__(self, fname="mnistmodel", fdir=join(".", "tmp")):
		self.graph = tf.Graph()
		fpath = abspath(join(fdir, fname))
		self.fname = fname
		self.dirname = abspath(fdir)
		self.fpath = fpath
		self.fname_flag = "trained_flag"
		self.fdir_flag = abspath(join(self.dirname, self.fname_flag))

		# if the directory doesn't exist, create it
		# TODO same as os.makedirs(path,exist_ok=True)?
		if not exists(self.dirname):
			# use a try in case of race condition
			try:
				makedirs(self.dirname)
			except OSError as e:
				if exception.errno != errno.EEXIST:
					raise IOError("Could not create directory for NNetMnist checkpoints.")
	...
	def fit(self, epochs=20000, batch_size=32, dropout_rate=0.5, verbose=True,
		...
		if skip_if_trained and not force_retrain:
			if isfile(self.fdir_flag):
				return 0
			# create file flag if not exist
			# TODO is there a better way to do this using os?
			with open(self.fdir_flag, 'w') as f:
				pass
